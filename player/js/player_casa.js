...
function playerAudio(e,box,xml,html5){
	...
	this.status_replay = this.status_random = this.next = this.playing = this.statusLoading = this.trackbar_status = false;
	...
	this.closed = this.many = this.empty = false;
	this.margin_box = 5;
	...
	this.resetPlayer = function(){
		...
		this.player_track_replay.getElementsByTagName("a")[0].className = this.btn_delete.className = this.btn_save.className = 'switch';
		this.btn_save.status = this.status_random = false;
		this.counter_tracks.style.display = "none";
		this.tracks = this.tracks_backup = this.arrayTracks = this.tracksExternal = new Array();
	...
	this.activePlayer = function(){
		...
		this.btn_delete.className = '';
		this.counter_tracks.style.display = "block";
	}
	...
	this.setElements = function(){
		...
		this.btn_new_playlist.onclick = function(){
			alert("Guardo una nueva playlist");
		}
	...
	this.removeTrack = function(block,btclose){
		...
		//console.log(block.id +" - "+ root.tracks[k].id +" | "+root.tracks.length);
		...
		function fade(){
			...
			if(root.tracks.length > 0){
				...
				root.counter_tracks.style.display = "none";
				root.resetPlayer();
			}
		...
		for(var i=0;i<master.length;i++){
			...
			box_link.appendChild(bt_close);
				...
				if(master[i].name.length > 15){
					content_name+= master[i].name.slice(0,13) + "...";
				}else{
			...
			if(master[i].artist.length > 15){
				content_artist+= master[i].artist.slice(0,13) + "...";
			}else{
	...
	this.setCorrectId = function(){
		...
		this.counter_tracks.innerHTML = this.tracks.length;
	...
	this.insertTrack = function(element,position,many){
		...
		//root.current_block.style = null;
		
		this.current_block.style.position = "relative";
		this.current_block.style.top = this.current_block.style.left = "auto";
